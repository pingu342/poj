...
// この配列数は入力画像では多くても1000であるが、出力では1000を超える場合がある
//
// ---
// 入力画像(幅7)の例
//
...
//      -1  -1  -1  -1  -1  -1  -1 -1 <-仮想的な行
// 
// 入力であるRLEペア(値,連長)を1002個保存する配列を確保し、仮想的な行も含めた全てを配列にロードする
//
// デコーダを3つ用意する
// ここでは例として画像の幅は7としている
// 画像の1行目の先頭をピクセル0とする
// 各デコーダの初回デコード位置を以下のピクセルにセットする
// 
//    デコーダ1 : ピクセル-8
//    デコーダ2 : ピクセル-1
//    デコーダ3 : ピクセル6
//
// 初回のデコードでは、初回デコード位置から3ピクセルをデコードし、結果を配列であるin[9]に保持する
//
//    デコーダ1 : ピクセル-8,-7,-6をデコードし、結果をin[0]~in[2]に保存
//    デコーダ2 : ピクセル-1,0,1をデコードし、結果をin[3]~in[5]に保存
//    デコーダ3 : ピクセル6,7,8をデコードし、結果をin[6]~in[8]に保存
//
// 仮想的なピクセル(上の例ではピクセル-8〜-1)のデコード結果は-1とする
//
// ピクセル0(すなわちin[4])の周りのピクセルとの差分の最大を求めることで、ピクセル0に対するエッジ検出の結果が得られる
// 差分を求める際、値が-1のピクセルは無視する
//
// 次のピクセル1に対するエッジ検出の結果を得るには、ピクセル0の際に得られている配列inの要素コピーと、各デコーダで1ピクセルのデコードを行い、ピクセル0(すなわちin[4])の周りのピクセルとの差分の最大を求めればよい
//
